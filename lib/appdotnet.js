...
AppDotNet.prototype.retrieveCreatedPosts = function (id, filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/users/' + id + '/posts' + objtoquery(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveMentions = function (id, filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/users/' + id + '/mentions' + objtoquery(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrievePersonalStream = function (filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/stream' + objtoquery(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveGlobalStream = function (filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/stream/global' + objtoquery(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveTaggedPosts = function (tag, filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/tag/' + tag + objtoquery(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
