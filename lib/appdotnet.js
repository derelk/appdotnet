...
AppDotNet.prototype.retrievePostReplies = function (id, filters, cb) {
  ...
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/' + id + '/replies' + '?' + querystring.stringify(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveCreatedPosts = function (id, filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/users/' + id + '/posts' + '?' + querystring.stringify(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveMentions = function (id, filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/users/' + id + '/mentions' + '?' + querystring.stringify(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrievePersonalStream = function (filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/stream' + '?' + querystring.stringify(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveGlobalStream = function (filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/stream/global' + '?' + querystring.stringify(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.retrieveTaggedPosts = function (tag, filters, cb) {
  var opts = {
    uri: AppDotNet.host + AppDotNet.version + '/posts/tag/' + tag + '?' + querystring.stringify(filters),
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
