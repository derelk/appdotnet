...
AppDotNet.prototype.muteUser = function (id, cb) {
  var opts = {
    uri: AppDotNet.host + '/users/' + id + '/mute',
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.post(opts, function (err, resp, body) {
    if (err) { return cb(err); }
...
AppDotNet.prototype.unmuteUser = function (id, cb) {
  var opts = {
    uri: AppDotNet.host + '/users/' + id + '/mute',
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
...
/**
 * Retrieve muted users and callback `cb(err, user)`.
 *
 * @param {Function} cb
 */

AppDotNet.prototype.retrieveMutedUsers = function (id, cb) {
  var opts = {
    uri: AppDotNet.host + '/users/me/muted',
    headers: { 'Authorization': 'Bearer ' + this.token }
  };
  request.get(opts, function (err, resp, body) {
    if (err) { return cb(err); }
